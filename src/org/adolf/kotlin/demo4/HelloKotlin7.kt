...
/*
    ...
    getValue与setValue方法既可以作为委托类的成员方法实现,也可作为其扩展方法来实现
    ...
    方法。
    委托转换规则:
    对于每个委托属性来说,Kotlin扁译器在底层会生成一个辅助的属性,然后将对原有属性的访问委托给这个辅助属性。
...
class People {
    val name: String by PeopleLauncher() //1  这里会生成两个PeopleLauncher对象
    val address: String by PeopleLauncher()  //2
}
